# FootCarbonPrint — Functional Specification & Design Requirements

> **Purpose of this document.** This is a complete, category-by-category inventory of everything FootCarbonPrint does (and is planned to do), written as **functional requirements**, **non-functional requirements**, and **design recommendations**. It is the single source of truth for a full UI/UX redesign. At the end you will find a **ready-to-paste prompt** for an AI prototyping tool (Figma AI, v0, Lovable, Uizard, Galileo, etc.) that will generate the full prototype space of the app.
...
- 🔶 **Partial** — a basic version exists; the full vision is not built.
- 🟡 **Planned** — described in the product vision (README) but not yet built.
...
| ESG-minded organizations 🟡 | Team/panel view for measuring collective impact. |
...
| FR-1.7 | Capture initial profile (country/region, urban/rural, household size, motivation). | 🟡 |
| FR-1.8 | One-tap registration/login (email, Google, Apple). | 🟡 |
...
| FR-2.9 | **Home/Energy module:** energy source, monthly kWh, household members, A/C usage hours. | ✅ |
...
| FR-2.17 | OCR bill scanning to auto-fill energy consumption (kWh) from a photo. | 🟡 |
| FR-2.18 | GPS/Maps sync to auto-detect transport mode and distance. | 🟡 |
| FR-2.19 | Per-question contextual tooltips ("Why does this matter?"). | 🟡 |
...
| FR-6.7 | Scheduled recurring re-test reminders (monthly/quarterly/annual). | 🟡 |
| FR-6.8 | Per-category trend history (not just total). | 🟡 |
...
| FR-7.11 | External resource link and automatic reminder per committed action. | 🟡 |
| FR-7.12 | Real-time "emissions gauge" that drops as commitments are fulfilled. | 🔶 |
...
| FR-9.4 | Show a challenges list in the Action Plan (with local check-off). | ✅ |
| FR-9.5 | Group challenges (team/community challenges). | 🟡 |
...
| FR-10.6 | Daily tips on Home / micro-content library. | 🟡 |
...
| FR-11.4 | In-app offset purchase / "green wallet" that converts EcoPoints into donations. | 🟡 |
...
| FR-12.5 | Direct-to-social posting with pre-composed graphics. | 🟡 |
...
| FR-14.4 | Reset all data to start over. | ✅ (engine) / 🔶 (needs a UI entry point) |
| FR-14.5 | Dedicated Settings screen consolidating all preferences. | 🟡 |
...
| FR-15.4 | Optional cloud sync / backup / multi-device. | 🟡 |
| FR-15.5 | Explicit data export/delete controls (privacy). | 🟡 |
...
| FR-16.2 | Auto-select language from device locale, falling back to English. | ✅ |
| FR-16.3 | Localize category names, level names, badge names/descriptions, and challenge copy. | ✅ |
| FR-16.4 | In-app language switcher. | 🟡 |
| FR-16.5 | Full accessibility pass (screen-reader labels, dynamic text scaling, contrast targets). | 🔶 |

---

## 4. Proposed New Functionalities

Beyond the vision already captured in FR items marked 🟡, these are **additional opportunities** surfaced while auditing the app. They are recommendations for the redesign to consider.

| ID | Proposed feature | Rationale |
|---|---|---|
| NEW-1 | **Dedicated Settings screen** consolidating theme, language, name, units, data reset, privacy, and about. | Preferences are currently scattered across app bars; a redesign should centralize them. |
| NEW-2 | **Goal-setting & target tracking** — let users set a personal reduction target and track progress toward it on the dashboard. | Turns passive history into an active commitment loop. |
| NEW-3 | **Achievements / Trophy room screen** — a full gallery of badges, levels, and milestones (not just inline cards). | Gamification currently lacks a "home" for pride/collection. |
| NEW-4 | **Action Plan progress dashboard** — track committed actions over time and show cumulative CO₂ avoided. | Closes the loop between committing and measurable outcome (partly FR-7.12). |
| NEW-5 | **Empty/first-run coaching states** with illustrations across all tabs. | Improves the cold-start experience consistently. |
| NEW-6 | **Notifications & reminders center** (weekly challenge, re-test, streak). | Retention driver; complements FR-6.7. |
| NEW-7 | **Streaks** for weekly challenges and check-ins. | Proven habit mechanic that fits the existing challenge system. |
| NEW-8 | **Category deep-dive screens** — tap a category to see its drivers, tips, and personal trend. | Makes the breakdown chart actionable. |
| NEW-9 | **Units & region preferences** (metric/imperial, currency for offset, country baseline). | Needed for international rollout (ties to FR-3.9). |
| NEW-10 | **Shareable weekly/monthly recap card** ("your month in CO₂"). | Extends the strong sharing feature into a recurring loop. |
| NEW-11 | **Comparison with friends / community leaderboard (opt-in, anonymous).** | The social pillar from the vision, done privacy-first. |
| NEW-12 | **Onboarding personalization step** (country, household, motivation) feeding the engine. | Improves calculation accuracy from the first run (ties to FR-1.7). |
...
- Consistent **bottom navigation** (Home · Footprint/Dashboard · Plan); consider a 4th tab for Achievements or Settings in the redesign.

### 6.3 Interaction & Motion
...
### 6.4 UX Principles (carry into the redesign)
...
### 6.5 Redesign Opportunities to Address
- Consolidate scattered settings into a **Settings screen** (NEW-1).
- Give gamification a **dedicated Achievements screen** (NEW-3).
- Add **category deep-dive** screens to make the breakdown actionable (NEW-8).
- Strengthen **empty/first-run** states across all tabs (NEW-5).
- Design an explicit, safe **data reset / privacy** surface (FR-14.4/FR-15.5).

---

## 7. Traceability & Double Validation

Two-directional check requested: (A) every implemented capability in the code maps to a requirement, and (B) every requirement maps to a screen/behavior.

### 7.1 A → B: Code capability → Requirement
...
### 7.2 B → A: Requirement → Implementation status

- All FR items marked ✅ correspond to a concrete screen/behavior listed in §2.3 and §7.1.
- All FR items marked 🔶 exist partially (reset engine without UI entry; action gauge concept; accessibility baseline).
- All FR items marked 🟡 are vision items with **no** current implementation (auth, OCR, GPS, bank sync, push, backend/admin, social, in-app offset purchase, region selector, recurring reminders, in-app language switch).

**Result:** Every requirement is classified and none is orphaned. ✅

### 7.3 New features added after validation
Section 4 (NEW-1 … NEW-12) captures additional functionality identified during this double-validation pass, plus the 🟡 vision items are retained as forward-looking requirements so nothing from the product concept is lost.

---

## 8. Category Index (Quick Reference)
...
17. **Proposed New Functionality** — NEW-1 … NEW-12

---

## 9. Ready-to-Paste Prototyping Prompt

> Copy everything inside the box below and paste it into your AI prototyping tool (Figma AI, v0, Lovable, Uizard, Galileo AI, etc.) to generate the full prototype space for FootCarbonPrint.

```text
You are a senior product designer. Design a complete, high-fidelity, mobile-first
prototype space for a cross-platform app called "FootCarbonPrint" (tagline:
"Know your footprint. Change your world."). It is a carbon-footprint self-diagnosis
app that turns everyday habits into awareness and concrete action. Generate ALL
screens, states, and flows listed below as a connected, clickable prototype.

BRAND & VISUAL SYSTEM
- Tone: nature-forward, optimistic, credible, non-judgmental (framing around
  possibility, never guilt).
- Primary green #1B5E20 → #2E7D32; secondary #66BB6A; light background #F1F8E9.
- Category colors (use consistently everywhere): Transport #1565C0, Food #6A1B9A,
  Home/Energy #E65100, Shopping #00695C, Waste #4E342E, Water #0277BD.
- Footprint level scale, always paired with an emoji/icon (never color alone):
  Champion 🌿 (green), Conscious 🌍 (blue), On Track 🟡 (yellow #F9A825),
  High Impact 🔴 (orange #E65100), Critical 💀 (red #B71C1C).
- Typography: clean geometric sans (Inter). Oversized, bold hero numbers for CO₂
  values; strong weight contrast between body and stats.
- Provide BOTH light and dark themes for every screen.
- Components: rounded elevated cards (radius 18–24), stat tiles, progress bars,
  interactive donut chart, line chart, choice chips, sliders, +/- counters,
  gradient hero headers, level badges, bottom navigation.
- Motion: subtle, delightful, respects reduced-motion (confetti on results,
  smooth gradient/level transitions, animated badges).

SCREENS TO GENERATE
1. Splash — animated logo + tagline.
2. Onboarding — 3-slide carousel (measure your impact / visual dashboard /
   achievements), skippable, animated page dots.
3. Main shell — bottom navigation with 3 tabs: Home, Dashboard (Footprint),
   Action Plan. (Also propose a 4th tab: Achievements or Settings.)
4. Home — greeting with avatar/name; eco-level summary card (level, points,
   progress to next level); achievements grid (8 badges, earned vs locked);
   rotating weekly challenge card (mark done, +25 pts, expires hint);
   test-progress card (X/6 modules). Include an EMPTY first-run state.
5. Test Hub — 2-column grid of 6 modules (Transport, Food, Home/Energy,
   Shopping, Waste, Water), each showing its % weight and completion check;
   overall progress bar; an educational "Learn" banner.
6. Six Test Modules — one screen each, all sharing the same scaffold: title,
   category color header, question cards with swipe/tap inputs (choice chips,
   sliders, +/- counters — no keyboards), and a LIVE CO₂ estimate preview that
   updates with input. Inputs per module:
   - Transport: vehicle type (8 options), weekly km slider, flights by
     short/medium/long counters.
   - Food: diet type, red-meat servings/week, local food, food waste.
   - Home/Energy: energy source, monthly kWh, household members, A/C hours.
   - Shopping: new clothing/month, electronics/year, online packages/month,
     second-hand toggle.
   - Waste: recycling toggle, compost toggle, waste bags/week.
   - Water: shower minutes, water temperature, garden watering.
7. Results — celebratory (confetti); giant total tCO₂ with level badge;
   comparison bars (You vs Paris target vs Colombia vs World); Top 3 emission
   sources ranked with % ; inline achievements (points/level/badges);
   CTAs: View Dashboard, View Action Plan, Share.
8. Dashboard — level-colored header with total; equivalences stat tiles
   (trees, light-bulb years, phone charges); interactive donut breakdown of 6
   categories with legend; benchmark bars (You/Paris/Colombia/World); history
   line chart with improved/worsened % delta (design empty, single-measurement,
   and multi-measurement states); carbon-offset calculator card (trees needed,
   annual + monthly cost, "reduce first" warning, links to certified platforms);
   buttons to Export Result image and open the Eco Card.
9. Action Plan — header with total potential reduction (t and %); a
   personalized, ranked list of ~6 recommended actions (each with emoji,
   title, description, category tag, −tCO₂ saving) with a Commit/Committed
   toggle (+15 pts); a weekly-challenges checklist. Include an EMPTY state.
10. Learn Guide — 6-lesson swipeable carousel (climate basics, greenhouse
    effect, footprints, categories, country comparison, actions); each lesson:
    big emoji, title, body, a highlighted "fact" callout, and a custom
    infographic visual; progress bar + prev/next; finish CTA.
11. Profile / Eco Card — a beautiful shareable identity card (avatar or photo,
    name, level badge, points, stats: CO₂ / X6 modules / X8 badges, decorative
    hex background, brand footer) with Add Photo and Download/Share PNG actions.
12. Export Result — bottom sheet previewing a branded result image card with a
    Download/Share button.
13. Settings (new) — theme (System/Light/Dark), language (English, Spanish,
    Portuguese, German), edit name, units/region, reset all data, privacy, about.
14. Achievements screen (new) — full gallery of badges, levels, and milestones.

CROSS-CUTTING REQUIREMENTS
- Design empty, loading, and completed states for every data-driven screen.
- Everything works offline and privacy-first (local data; sharing is explicit).
- Fully responsive (small phone → large/web), no horizontal overflow.
- Accessible: WCAG AA contrast, 44pt touch targets, scalable text, meaning never
  conveyed by color alone.
- Localizable layouts (tolerate ±30% text length).
- Deliver a component library / design system alongside the screens, and connect
  the screens into the primary flow:
  Splash → Onboarding → Home → Test Hub → Modules → Results → Dashboard →
  Action Plan → (Learn, Eco Card, Settings, Achievements).
```
